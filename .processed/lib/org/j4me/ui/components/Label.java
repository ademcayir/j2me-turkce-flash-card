	...
	extends Component
{
	 ...
	 */
	protected Font getFont (Theme theme)
	{
		if ( font != null )
		{
			return font;
	 ...
	 */
	protected int getFontColor (Theme theme)
	{
		if ( fontColor >= 0 )
		{
			return fontColor;
			...
			for ( int i = 0; i < lines.length; i++ )
			{
				...
				else
				{
					if ( (y + fontHeight >= clipTop) && (y <= clipBottom) )
					{
						...
						if (isUnderlined){
							int ww = font.stringWidth(lines[i]);
							int underlined_x = x;
							...
							g.drawLine(underlined_x, y+font.getHeight()+1, underlined_x+ww, y+font.getHeight()+1);
						}
					}
	...
	public void setUnderlined(boolean isunderlined){
		if (this.isUnderlined != isunderlined){
